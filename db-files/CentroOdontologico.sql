...
ALTER TABLE CITA ADD CONSTRAINT CITA_CHK1 CHECK ( COSTO>=0) ;
...
AND REGEXP_LIKE(TELEFONO,'[0-9]+')) ENABLE;
CREATE UNIQUE INDEX DOCTOR_PK1 ON MEDICO(NUM_COLEGIO ASC, RIF ASC);
...
--------------------PROCEDIMIENTO --------------------------
/*
	DEVUELVE TRATAMIENTO CON MAYOR PORCENTAJE DE GANANCIA CON RESPECTO
	A LAS GANANCIAS TOTALES
*/
/*
create or replace PROCEDURE EJECUTAR 
(
  MES IN VARCHAR2,
  SALIDA OUT VARCHAR2
) AS 
CURSOR C_TRATAMIENTOS(V_MES VARCHAR2) IS
SELECT * FROM CITA_TRATAMIENTO JOIN TRATAMIENTO ON(TRATAMIENTO_ID=ID) WHERE TRIM(TO_CHAR(FECHA,'MONTH'))=UPPER(V_MES) ORDER BY DESCRIPCION;
V_IMPLEMENTOS NUMBER;
V_GANANCIA NUMBER:=0;
V_NOMBRE_MAX VARCHAR2(100);
V_GAN_MAX NUMBER:=-1;
V_GAN_AUX NUMBER:=0;
TRATAMIENTO_ACT VARCHAR2(100):=' ';
BEGIN
  V_GANANCIA:=GANANCIA(MES);
  ...
  FOR FILA_CURSOR IN C_TRATAMIENTOS(MES) LOOP
    ...
    IF TRATAMIENTO_ACT!=FILA_CURSOR.DESCRIPCION THEN

      V_GAN_AUX:=ROUND(V_GAN_AUX/V_GANANCIA*100,2);
      IF V_GAN_AUX>V_GAN_MAX THEN
        V_GAN_MAX:=V_GAN_AUX;
        V_NOMBRE_MAX:=TRATAMIENTO_ACT;
      END IF;
      TRATAMIENTO_ACT:=FILA_CURSOR.DESCRIPCION;
      V_GAN_AUX:=FILA_CURSOR.ABONADO*(1-FILA_CURSOR.PORCE_GANAN_MEDICO)-V_IMPLEMENTOS;
    ELSE
      V_GAN_AUX:=V_GAN_AUX+FILA_CURSOR.ABONADO*(1-FILA_CURSOR.PORCE_GANAN_MEDICO)-V_IMPLEMENTOS;
    END IF;
  ...
  IF V_GAN_AUX>V_GAN_MAX THEN
    ...
    V_NOMBRE_MAX:=TRATAMIENTO_ACT;
  END IF;
  SALIDA:=V_NOMBRE_MAX||'|'||V_GAN_MAX;
END EJECUTAR;

--PARA LLENAR CAMPO DE CI_MEDICO A CITA_TRATAMIENTO

CREATE OR REPLACE PROCEDURE LLENAR5 AS 

TYPE VECTOR IS TABLE OF NUMBER INDEX BY BINARY_INTEGER;

CURSOR C_ESP_MEDICO(V_CI NUMBER) IS
SELECT DISTINCT ID_ESPECIALIZACION FROM ESPECIALIZACION_MED_EMP WHERE CI_MEDICO=V_CI;
CURSOR C_ESP_TRATAMIENTO(V_ID NUMBER) IS
SELECT ESPECIALIZACION_ID FROM ESPECIALIZACION_TRATAMIENTO WHERE TRATAMIENTO_ID=V_ID;
I NUMBER:=0;
BAN NUMBER:=0;
V_ESP_TRAT VECTOR;

CURSOR CUR_C IS SELECT CITA_ID, TRATAMIENTO_ID FROM CITA_TRATAMIENTO;
  CURSOR CUR_M IS SELECT CI FROM MEDICO;

  N_RANDOM NUMBER;
  NCOUNT NUMBER;

BEGIN
    FOR REC_C IN CUR_C LOOP
  
      I := 0;
      BAN := 0;
      v_esp_trat.delete;
      
      FOR FILA IN C_ESP_TRATAMIENTO(REC_C.TRATAMIENTO_ID) LOOP
            V_ESP_TRAT(I):=FILA.ESPECIALIZACION_ID;
            I:=I+1;
      END LOOP;
          
      FOR REC_M IN CUR_M LOOP
          IF I>0 THEN
            FOR FILA IN C_ESP_MEDICO(REC_M.CI) LOOP
              FOR J IN 0..V_ESP_TRAT.COUNT-1 LOOP
                IF FILA.ID_ESPECIALIZACION = V_ESP_TRAT(J) THEN
                  BAN:=1;
                  DBMS_OUTPUT.PUT_LINE('MEDICO ES: '||REC_M.CI);
                  UPDATE CITA_TRATAMIENTO SET CI_MEDICO = TO_NUMBER(REC_M.CI) WHERE CITA_ID = REC_C.CITA_ID;
                  EXIT;
                END IF;
              END LOOP;
              EXIT WHEN BAN=1;
          ...
          END IF;
          IF BAN=0 THEN
            DBMS_OUTPUT.PUT_LINE ('El medico no esta especializado para este tipo de tratamiento');
          END IF;
      END LOOP;
    END LOOP;
END LLENAR5;
...
------FUNCION GANANCIA POR MES Y POR AÑO--------------
...
SELECT * FROM CITA_TRATAMIENTO JOIN TRATAMIENTO ON(TRATAMIENTO_ID=ID) WHERE TRIM(TO_CHAR(FECHA,'MONTH'))=UPPER(MES_I) AND TO_CHAR(FECHA,'YYYY')=ANO;
V_IMPLEMENTOS NUMBER;
V_CITAS NUMBER;
V_GANANCIA NUMBER:=0;
BEGIN
  ...
  END LOOP;
  SELECT NVL(SUM(COSTO),0) INTO V_CITAS FROM CITA WHERE TRIM(TO_CHAR(FECHA,'MONTH'))=UPPER(MES) AND TO_CHAR(FECHA,'YYYY')=ANO;
  V_GANANCIA:=V_GANANCIA+V_CITAS;
  RETURN V_GANANCIA;
END GANANCIA;

-----------FUNCION QUE DEVUELVE GANANCIAS DE LOS MESES DE UN AÑO PARA GRAFICAR-------------------------
--------------DEVUELVE UNA CADENA:  |GANANCIADEENERO|GANANCIA...|GANANCIADEDICIEMBRE|--------------

CREATE OR REPLACE FUNCTION GRAFICOANIO 
(
  ANIO IN VARCHAR2 
) RETURN VARCHAR2 AS 
RESPUESTA VARCHAR2(240):='|';
AUX VARCHAR2(20);
BEGIN
  FOR I IN 1..12 LOOP
    SELECT GANANCIA(TRIM(TO_CHAR(TO_DATE(I,'MM'),'MONTH')),ANIO) INTO AUX FROM DUAL;
...
---------------------------PORCENTAJE DE TRATAMIENTOS EN UN AÑO---------------------------
--------------------DIAGRAMA DE TORTAS---------------------------
----------------DEVUELVE CADENA: |NOMBRE_TRATAMIENTO:PORCENTAJE|NOMBRE_TRATAMIENTO2:PORCENTAJE2|.....|

CREATE OR REPLACE FUNCTION PORCENTAJE_TRATAMIENTOS 
(
  ANIO IN VARCHAR2 
) RETURN VARCHAR2 AS
...
TOTAL NUMBER;
PORCENTAJE NUMBER;
CURSOR TRATAMIENTOS(V_ANIO NUMBER) IS
SELECT DISTINCT T.DESCRIPCION NOMBRE FROM CITA_TRATAMIENTO CT JOIN TRATAMIENTO T
ON(T.ID=CT.TRATAMIENTO_ID)
WHERE TO_CHAR(FECHA,'YYYY')=V_ANIO;
BEGIN
  SELECT NVL(COUNT(*),0) INTO TOTAL FROM CITA_TRATAMIENTO
  WHERE TO_CHAR(FECHA,'YYYY')=ANIO;
  
  FOR FILA IN TRATAMIENTOS(ANIO) LOOP
    SELECT NVL(COUNT(*),0) INTO PORCENTAJE FROM CITA_TRATAMIENTO CT JOIN TRATAMIENTO T
    ON(T.ID=CT.TRATAMIENTO_ID)
    WHERE TO_CHAR(FECHA,'YYYY')=ANIO AND T.DESCRIPCION=FILA.NOMBRE;
    PORCENTAJE:=ROUND(PORCENTAJE/TOTAL*100,2);
    RESPUESTA:=RESPUESTA||FILA.NOMBRE||':'||PORCENTAJE||'|';
  END LOOP;
  RETURN RESPUESTA;
END PORCENTAJE_TRATAMIENTOS;
*/
/*
...
----------------TRRRRIIIIGGGGEERRRR--------------------------
	...
	VALIDA QUE LOS MEDICOS ESTEN ESPECIALIZADOS PARA ATENDER EL TRATAMIENTO


CREATE OR REPLACE TRIGGER MEDICO_ESPECIALIZADO 
BEFORE INSERT OR UPDATE ON CITA_TRATAMIENTO 
FOR EACH ROW 

DECLARE TYPE VECTOR IS TABLE OF
NUMBER
...
BEGIN
  IF INSERTING OR UPDATING('CI_MEDICO') OR UPDATING('TRATAMIENTO_ID') THEN
    FOR FILA IN C_ESP_TRATAMIENTO(NVL(:NEW.TRATAMIENTO_ID,:OLD.TRATAMIENTO_ID)) LOOP
      V_ESP_TRAT(I):=FILA.ESPECIALIZACION_ID;
      I:=I+1;
    END LOOP;
    
    IF I>0 THEN
      FOR FILA IN C_ESP_MEDICO(NVL(:NEW.CI_MEDICO,:OLD.CI_MEDICO)) LOOP
        FOR J IN 0..V_ESP_TRAT.COUNT-1 LOOP
          IF FILA.ID_ESPECIALIZACION = V_ESP_TRAT(J) THEN
            BAN:=1;
            EXIT;
    ...
    END IF;
    IF BAN=0 THEN
      RAISE_APPLICATION_ERROR(-20000, 'El medico no esta especializado para este tipo de tratamiento');
    END IF;
  END IF;
END;
